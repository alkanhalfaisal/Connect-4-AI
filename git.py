...
class Game:
    ...
    def is_game_over(self):
        if self.has_winning_state():
            """Display who won"""
            print("AI Bot won!") if ~self.turn == self.AI else print(
    ...
    def query_player(self):
        ...
        self.current_state = State(
            self.current_state.ai_position, new_game_position, self.current_state.depth + 1)

    def query_AI(self):
        """ AI Bot chooses next best move from current state """
        print("\nAI's Move...")
        temp_position = self.current_state.ai_position
        self.current_state = alphabeta_search(
            self.current_state, self.first, d=7)
        ...
        for i in range(0, 7):
            ...
            if not self.game_position & (1 << (7 * column + 5)):
                if (who_went_first == -1 and self.depth % 2 == 0) or (who_went_first == 0 and self.depth % 2 == 1):
                    # AI (MAX) Move
                    new_ai_position, new_game_position = make_move(
                        self.ai_position, self.game_position, column)
                else:
                    # Player (MIN) move
...
def alphabeta_search(state, turn=-1, d=7):
    ...
    cutoff_search = (lambda state, depth: depth >
                     d or state.terminal_node_test())
    best_score = -infinity
    beta = infinity
    best_action = None
    for child in state.generate_children(turn):
