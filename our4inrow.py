...
def getAllPossibleMoves(board):
    ...
    columns = 7
    return [col for col in [3,2,4,1,5,0,6] if findValidMove(col, board)[0]]
...
def evaluateMove(board,player):
    """This method will evaluate each AI move"""
    score =0


def calculateBestMove(board, depth, isMax, difficulty , alpha, beta,possibleMove = 3):
    ...
    if isMax:
        ...
        for possibleMove in getAllPossibleMoves(board):
            ...
            if row != -1:
                score = calculateBestMove(board, depth-1, False, difficulty, alpha, beta,possibleMove) - 1
